...
``Message.meta`` is a JSON column, ``jsonb`` on Postgres. What hangs off a
message varies by what produced it (attached filings, the run that answered,
the reason a run failed) and none of it is queried structurally, so it lives
there rather than as a column each new kind of message would add.
"""
...
from models.columns import utcnow
...
def _json_column(name: str = "metadata") -> Column:
    """A JSON column, stored as ``jsonb`` where the backend has it.

    ``jsonb`` is the reason this is worth a variant: Postgres stores it parsed
    and can index into it, while everything else falls back to plain JSON text.
    """
