...
:mod:`auth.schemas` inherit from — the email and name rules are written once
...
which is why :meth:`auth.service.AuthService.signup` builds rows that
...
wherever it happened to be touched. Postgres enforces the cascade itself, which
is the whole reason it is declared on the column.
"""
...
from db.columns import as_utc, utcnow
